#!/usr/bin/env bash
# Submify quickstart — macOS & Linux.
#
...
# get.docker.com script on Linux). Does the same for git if it's missing
# too. Then runs install.sh, which clones (or updates) Submify into
# ./Submify and starts the full stack.
#
# Usage:
...
# succeed, and install.sh updates an existing ./Submify checkout in place
# rather than clobbering it.
set -euo pipefail

INSTALL_URL="https://raw.githubusercontent.com/Raktim94/Submify/main/install.sh"
SUDO=""
...
echo "==> Installing and starting Submify..."
$SUDO bash -c "curl -fsSL $INSTALL_URL | bash"
