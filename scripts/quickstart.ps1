# Submify quickstart — Windows (PowerShell).
#
...
# quickstart.sh inside WSL2 to install and start Submify. It also installs
...
$distroName = "Ubuntu"
$repoDir = "Submify"
$linuxQuickstartUrl = "https://raw.githubusercontent.com/Raktim94/Submify/main/scripts/quickstart.sh"
...
if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        ...
        Write-Host "winget isn't available, so the native Windows 'docker' command will be skipped — Submify will still install and run fully inside WSL2." -ForegroundColor Yellow
    }
}

Write-Host "==> Installing and starting Submify inside WSL2 (Docker Engine and git are installed automatically in there if needed)..."
wsl.exe -d $distroName -u root -- bash -lc "curl -fsSL $linuxQuickstartUrl | bash"

Write-Host ""
Write-Host "Submify is up and running." -ForegroundColor Green
