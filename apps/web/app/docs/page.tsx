export default function DocsPage() {
    ...
    <>
      ...
      <section id="overview" className="animate-fade-in-up motion-reduce:animate-none">
          ...
          <p>
            ...
            <code>/api/v1</code> (for example <code>/api/v1/projects</code>). <strong>Public</strong> form submissions use a
            separate route: <code>POST /api/submit</code> and do not use your account password — they use the{' '}
            <strong>project public key</strong> (<code>pk_live_…</code>) in the <code>x-api-key</code> header.
          </p>
          <p>
            Typical flow: <strong>register</strong> → open <strong>Projects</strong> → create or select a project → copy the public
            (and secret, for server-side HMAC) keys → point your form or fetch script at your site&apos;s{' '}
            <code>/api/submit</code> → review <strong>Submissions</strong> and <strong>Export</strong> as needed.
          </p>
        ...
        <section id="architecture" className="animate-fade-in-up motion-reduce:animate-none [animation-delay:60ms]">
          ...
          <ul>
            ...
            <li>
              <strong>Object storage</strong> (S3-compatible, e.g. MinIO): optional presigned uploads for large files from the
              dashboard flow.
            </li>
            <li>
              <strong>Reverse proxy</strong> (nginx in the default stack): routes <code>/api/*</code> to the API and everything else
              to the web UI.
            </li>
          </ul>
        </section>
        ...
        <section id="quick-start" className="animate-fade-in-up motion-reduce:animate-none [animation-delay:120ms]">
          ...
          <ol>
            <li>
              <strong>Create an account</strong> — Register with name, phone, email, and password (8+ characters). You are signed in
              with JWT access + refresh tokens stored in the browser.
            </li>
            <li>
              <strong>Open Projects</strong> — You get a default inbox; you can create more projects. Each has a unique public/secret
              pair.
            </li>
            <li>
              <strong>Copy the submit URL</strong> — Shown on the Projects page (same path on every host:{' '}
        ...
        <section id="auth">
          ...
          <p>
            When the access token expires, the app refreshes it using <code>POST /api/v1/auth/refresh</code> with your refresh token.
        ...
        <section id="projects-keys">
          ...
          <p>
            Each <strong>project</strong> is an inbox with its own keys:
          </p>
          <ul>
            <li>
              <strong>Public key</strong> (<code>pk_live_…</code>) — Safe to embed in public frontends; used as{' '}
              <code>x-api-key</code> for <code>POST /api/submit</code>.
            </li>
            <li>
              <strong>Secret key</strong> (<code>sk_live_…</code>) — For HMAC signing on a server you trust; never expose in
              browser-only code.
            </li>
            <li>
              <strong>Allowed origins</strong> (optional) — JSON array of exact origins (e.g. <code>https://example.com</code>). If
              set, browser submissions may be restricted to those origins.
            </li>
          </ul>
          <p>
        ...
        <section id="submit-api">
          ...
          <ul>
            ...
            <li>
              <code>x-signature</code> — Optional. HMAC-SHA256 hex digest of the raw JSON body using the project secret, for
          ...
          <h3>Body</h3>
          <p>JSON object. Common shape includes <code>data</code> for fields and <code>files</code> for references; the API stores the
          payload and may notify Telegram if configured.</p>
          <pre>
            <code>{`{
  ...
  "files": []
        ...
        <section id="rate-limits">
          <h2>Rate limits</h2>
          <p>
            Sensitive public routes (login, register, etc.) and submit routes are rate-limited per IP and/or per key. If you hit a
            limit, wait briefly and retry. Self-hosted users can tune limits via environment variables (see below).
          </p>
        </section>
        ...
        <section id="dashboard">
          ...
          <p>
            The <strong>Dashboard</strong> shows API health, optional <strong>GitHub</strong> version checks (via{' '}
            <code>GITHUB_REPO</code>), and recent activity. The server compares tags/releases to your running <code>APP_VERSION</code>.
          </p>
          <p>
        ...
        <section id="submissions">
          ...
          <p>
            Per project, open <strong>Submissions</strong> to list rows. Use <strong>Export</strong> to download XLSX or PDF with a
            Bearer token. Bulk delete is available when you need to free space under the per-project cap.
          </p>
        </section>
        ...
        <section id="settings">
          ...
          <p>
            <strong>S3-compatible storage</strong> — Optional endpoint, keys, and bucket for presigned uploads when you need large
            files; small JSON submissions work without it.
          </p>
        </section>
        ...
        <section id="self-hosting">
          ...
          <p>
            The default stack uses <code>docker compose</code>: API, web, PostgreSQL, S3-compatible storage, nginx on a port (e.g.
            2512). Persist data via the documented volume paths. Set <code>JWT_SECRET</code> and database credentials in production.
          </p>
          <p>
            Mount the project directory and Docker socket into the API container only if you want the dashboard &quot;Update &amp;
            restart&quot; button to run compose on the host.
          </p>
        </section>
        ...
        <section id="env-vars">
          ...
          <p>
            Names and defaults may vary by release; check your <code>docker-compose.yml</code> and the API <code>config</code> package
            for the source of truth. Common values:
          </p>
          <table>
            <thead>
              <tr>
        ...
        <section id="troubleshooting">
          ...
          <ul>
            <li>
              <strong>401 / invalid token</strong> on dashboard — Session expired or server secret changed; sign in again. Ensure the
              web app is rebuilt so token refresh runs.
            </li>
            <li>
